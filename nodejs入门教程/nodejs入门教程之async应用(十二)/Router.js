...
class Router {
    ...
    handle(req, res) { // 处理req和res
        ...
        switch (request.pathname) { // 根据解析出来的pathname链接到用户信息模块路由的对应参数上
            case '/register': //注册
                this.userRoute.register(request, res).then();
                break
            case '/login': //登陆
                this.userRoute.login(request, res).then()
                break
            case '/changeNickname': //改昵称
                this.userRoute.changeNickname(request, res).then()
                break
            default: // 其他路由
                this.userRoute.handelOther(request, res).then();
                break
        }
